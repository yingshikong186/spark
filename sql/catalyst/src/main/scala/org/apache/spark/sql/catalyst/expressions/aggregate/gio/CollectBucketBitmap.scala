...
case class CollectBucketBitmap(
    ...
    override val inputAggBufferOffset: Int = 0) extends TypedImperativeAggregate[BucketBitMap] {
  ...
  override def update(buffer: BucketBitMap, input: InternalRow): Unit = {
  ...
  override def merge(buffer: BucketBitMap, input: BucketBitMap): Unit = {
    buffer.or(input)
  }
