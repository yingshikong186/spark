...
case class MergeCBitmap(
    ...
    override val inputAggBufferOffset: Int = 0) extends TypedImperativeAggregate[CBitMap] {
  ...
  override def update(buffer: CBitMap, input: InternalRow): Unit = {
    val _bitmap = new CBitMap(bitmap.eval(input).asInstanceOf[Array[Byte]])
    buffer.or(_bitmap)
  }

  override def merge(buffer: CBitMap, input: CBitMap): Unit = {
    buffer.or(input)
  }
